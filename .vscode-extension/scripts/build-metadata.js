...
if (fs.existsSync(navFile)) {
  ...
  for (const l of navLines) {
    ...
    if (m) { navTitles[parseInt(m[1], 10)] = m[2].trim(); }
  }
}

const metadata = {};
const dirs = fs.readdirSync(roomsDir).filter(d => /^room_\d+$/.test(d)).sort();
...
for (const dir of dirs) {
  ...
  const readmeTitle = titleLine ? titleLine.replace(/^#+\s*/, '').replace(/^Room\s+\d+\s*[-–]\s*/i, '').trim() : null;
  ...
  let currentTask = '';
  for (const l of lines) {
    if (/^##\s*Tasks/i.test(l))   { inTasks = true; continue; }
    if (/^##/.test(l) && inTasks) {
      if (currentTask) { tasks.push(currentTask.trim()); currentTask = ''; }
      break;
    }
    if (!inTasks) { continue; }
    if (/^\d+\./.test(l.trim())) {
  ...
  // Deduplicate
  const uniqueCommands = [...new Set(commands)].filter(c => c.includes(' ') || c.length > 3);

  metadata[num] = { number: num, title, section, tasks, hints, commands: uniqueCommands };
}

fs.writeFileSync(outFile, JSON.stringify(metadata, null, 2));
console.log(`rooms-metadata.json written — ${Object.keys(metadata).length} rooms`);

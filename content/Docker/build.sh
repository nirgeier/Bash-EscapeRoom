...
# The bake file referenced here is a docker-compose.yml that defines the
# build context and platforms. The service name targeted is
# `escape-room-bash` (see content/Docker/docker-compose.yml).
# The `--push` flag instructs Buildx to push the built images to the
# configured registry (Docker Hub by default for the IMAGE above).
echo "Running buildx bake (build + push)..."
